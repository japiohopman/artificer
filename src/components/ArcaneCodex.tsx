...
import { loginWithGoogle, logout } from './core/FirebaseProvider';
...
import { TitleScreen } from './core/TitleScreen';
...
const ArcaneCodex: React.FC = () => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Handle V2 Slot-to-Slot movement
    if (dropData?.slotId) {
      const sourceSlotId = dragData.slotId;
      const targetSlotId = dropData.slotId;
      const characterId = activeCharacterId;

      if (sourceSlotId && targetSlotId && characterId) {
        // Validation: If target is an equipment slot, check rules
        const targetSlotRules = EQUIPMENT_SLOT_CATALOG.find(s => s.id === targetSlotId);
        if (targetSlotRules) {
           const itemKind = item.kind;
           const isAllowed = !targetSlotRules.accepts.kinds || (itemKind && targetSlotRules.accepts.kinds.includes(itemKind));
        ...
        moveItemV2({ characterId, sourceSlotId, targetSlotId });
        soundService.playEffect('UI_ITEM_PLACE');
        return;
      }
    }

    // Handle transfer between characters or party inventory (Legacy fallback or cross-character)
    ...
    // Handle equip on equipment slot (Legacy fallback)
    if (dropData?.slot) {
          ...
          <div className="flex items-center gap-1 md:gap-3 overflow-x-auto no-scrollbar py-2">
            {!isProfileMenuOpen && (
              <>
                ...
                >
                  <GameIcon name="weapon" size={16} /> <span className="hidden lg:inline">Actions</span>
                </button>
                <button 
                  onClick={() => {
                    playClickSound();
                            ...
                            <div
                              ...
                              onClick={() => {
                                if ((explorerTab === 'equipment' || explorerTab === 'materials' || explorerTab === 'enemies' || explorerTab === 'spells') && !selectedCategory) {
                                  ...
                                  selectItem(m.index, explorerTab);
