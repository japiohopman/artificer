...
export const SpellbookReader: React.FC<SpellbookReaderProps> = ({ isOpen, onClose }) => {
        ...
        return {
          ...
          content: (
            <div className="space-y-6 font-serif text-[11px] leading-relaxed text-black/80 italic">
               ...
               {spell.higher_level && (
                  <div className="mt-6 p-4 border-l-2 border-black/10 bg-black/[0.01]">
                    <span className="font-bold uppercase text-[9px] tracking-widest opacity-40 block mb-2">At Higher Tiers:</span>
                    <p className="text-[10px] opacity-80">{Array.isArray(spell.higher_level) ? spell.higher_level.join(' ') : spell.higher_level}</p>
                  </div>
               )}
                  ...
                  {spell.level === 0 && (
                    <div className="flex flex-col items-center gap-2">
                       <div className="w-8 h-8 rounded-full border border-black/10 flex items-center justify-center">
                          <GameIcon name="lightning" size={14} color="#000" />
                       </div>
                       <span className="text-[9px] font-black uppercase tracking-widest opacity-40">Innate Manifestation</span>
                    </div>
                  )}
               </div>
            </div>
  ...
  return (
    <BookReader 
      ...
      className="z-[5000]"
    >
      <div className="absolute bottom-12 right-12 flex flex-col items-end gap-2 pointer-events-none opacity-40">
         <span className="text-[10px] font-black uppercase tracking-[0.3em]">Caster Matrix</span>
         <div className="flex gap-4">
            <div className="text-right">
               <span className="block text-[8px] font-bold uppercase tracking-widest leading-none">Save DC</span>
               <span className="text-2xl font-header font-black">{spellDC}</span>
            </div>
            <div className="text-right">
               <span className="block text-[8px] font-bold uppercase tracking-widest leading-none">Attack</span>
               <span className="text-2xl font-header font-black">+{attackBonus}</span>
            </div>
            <div className="text-right">
               <span className="block text-[8px] font-bold uppercase tracking-widest leading-none">Memory</span>
               <span className={cn("text-2xl font-header font-black", preparedIndices.length > maxPrepared ? "text-red-600" : "")}>
