  ...
  const activeChar = useMemo(() => {
    ...
    if (monster) {
      ...
      return {
        ...
        inventory: {},
        backpack: [],
        knownSpells: (monster as any).knownSpells || (monster as any).spells || [],
        preparedSpells: (monster as any).preparedSpells || ((monster as any).knownSpells || []).map((s: any) => s.index || s.id).filter(Boolean),
        spellSlots: (monster as any).spellSlots || { "1": { current: 2, max: 2 } },
        choices: {},
        hp: monster.hp,
        maxHp: monster.maxHp,
