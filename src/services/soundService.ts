...
class SoundEngine {
  ...
  playLayer(layerId: AudioLayer, path: string, loop: boolean = false) {
    ...
    const state = useAudioStore.getState().layerStates[layerId];
    audio.volume = state.isMuted ? 0 : state.volume * this.masterVolume;
    const anySolo = Object.values(useAudioStore.getState().layerStates).some(s => s.isSolo);
  ...
  updateLayerSolo(layerId: AudioLayer, isSolo: boolean) {
    ...
    const { layerStates } = useAudioStore.getState();
