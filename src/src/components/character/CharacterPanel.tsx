      ...
      <AnimatePresence>
        {isInventoryOpen && (
          <motion.div
            ...
            className="w-80 bg-parchment-50/50 border-l border-parchment-300 flex flex-col z-10 items-center justify-center p-8 text-center"
          >
            <Package size={48} className="text-parchment-300 mb-4" />
            <h2 className="text-lg font-header font-bold text-dragon-red uppercase mb-2">No Character Active</h2>
            <p className="text-xs text-parchment-500 italic">Please select or create a character to view inventory.</p>
            <button 
              onClick={() => setIsInventoryOpen(false)}
              title="Close Panel"
              aria-label="Close Panel"
              className="mt-6 px-4 py-2 bg-dragon-red text-white text-[10px] font-bold uppercase tracking-widest rounded-sm"
            >
              Close Panel
            </button>
          </motion.div>
              ...
              {activeTab === 'inventory' && (
                <Inventory 
                  onEquipRequest={(item) => {
                    ...
                    const targetSlot = slots.find((s: string) => !inventory[s]) || slots[0];
