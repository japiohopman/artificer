...
export const CategoryIcons: Record<string, { icon: any; color: string }> = {
  city: { icon: 'castle', color: '#60A5FA' },
  dungeon: { icon: 'skull', color: '#F87171' },
  forest: { icon: 'tree', color: '#34D399' },
  shop: { icon: 'shopping_cart', color: '#FBBF24' },
  tavern: { icon: 'coffee', color: '#F87171' },
  entrance_exit: { icon: 'door_open', color: '#A78BFA' }
};
...
interface AppState {
  ...
  setPartySubLocation: (pos: { x: number; y: number }) => void;
  playSound: (soundId: string) => void;
  getActiveBackground: () => string | null;
  isNight: () => boolean;
  ...
  setInspectingItem: (data: { item: any; sourceId: string; itemId?: string; index?: number; slot?: string; } | null) => void;
...
export const useStore = create<AppState>((set, get) => ({
  ...
  setIsAdvancedRollerOpen: (isOpen) => set({ isAdvancedRollerOpen: isOpen }),

  // Game World Init
  gameTime: 600, // 10:00 AM
  gameDay: 1,
  currentLocation: null,
  currentSubLocation: null,
  partyLocation: { x: 50, y: 50 },
  savedLocations: [],
  isInsideSubMap: false,
  partySubLocation: { x: 50, y: 50 },
  beastRegistry: {},
  ...
  addLog: (message, type = 'info') => set((state) => ({
    logs: [...state.logs, { id: crypto.randomUUID(), message, type, timestamp: Date.now() }]
  })),
  clearLogs: () => set({ logs: [] }),
  ...
  playSound: (soundId) => {
    import('../services/soundService').then(({ soundService }) => {
      soundService.playEffect(soundId as any);
    });
  },
  getActiveBackground: () => {
    const { currentLocation, currentSubLocation, isNight } = get();
    // Logic to return a background URL based on location and time
    // For now, return a placeholder or mock
    if (currentLocation?.overlayMapUrl) return currentLocation.overlayMapUrl;
    return 'https://raw.githubusercontent.com/japiohopman/artificer/main/public/assets/atlas/maps/world/toril_political.webp';
  },
  isNight: () => {
    const { gameTime } = get();
    return gameTime < 360 || gameTime > 1200; // Night is before 6am or after 8pm
  },
